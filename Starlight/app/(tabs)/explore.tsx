...
export default function NowPlayingScreen() {
  ...
  const durationMs = Number.isFinite(activeTrack?.durationMs ?? 0) ? (activeTrack?.durationMs ?? 0) : 0;
  const safePosition = Number.isFinite(positionMs) && positionMs >= 0 ? positionMs : 0;
  const remaining = Math.max(durationMs - safePosition, 0);

  return (
    <View style={styles.container}>
      <Surface variant="elevated" padding="lg" style={styles.card}>
        <View style={[styles.artworkPlaceholder, { backgroundColor: tokens.colors.primaryMuted }]}> 
          <IconSymbol name="house.fill" color={tokens.colors.primary} size={64} />
        </View>
        <Text variant="subtitle" weight="bold">
          {activeTrack?.title ?? 'No track playing'}
        </Text>
        <Text tone="subtle">{activeTrack?.artist?.name ?? 'Select a track to begin playback'}</Text>
        <View style={styles.timeline}>
          <Text variant="caption" tone="subtle">
            {formatTime(safePosition)}
          </Text>
          <View style={[styles.timelineBar, { backgroundColor: tokens.colors.surfaceElevated }]}>
            <View
              style={[
                styles.timelineProgress,
                {
                  width: timelineProgressPercent(safePosition, durationMs),
        ...
        <View style={styles.controls}>
          <IconButton
            icon={<IconSymbol name="chevron.right" size={20} style={styles.prevIcon} />}
